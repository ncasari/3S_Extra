...
def Compressor(ScrollClass, Te = 273, Tc = 300, f = None, OneCycle = False, Ref = 'R454B.MIX', HDF5file='scroll_compressor_R454B.h5'):
    ...
    inletState = State.State(Ref,dict(T=Tin,P=pe))

    print("S1 - in")
    print(inletState.s)
    T2s = ScrollComp.guess_outlet_temp(inletState, pc)
    outletState = State.State(Ref,dict(T=T2s,P=pc))
