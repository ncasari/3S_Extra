...
#                    raise
    ...
    def auto_add_CVs(self,inletState,outletState):
        ...
        for alpha in range(1,nCmax+1):
            ...
            if alpha==1:
                ...
                # Assume adiabatic compression from the inlet state at the end of the suction process.
                # As a first assumption, ideal gas should work
                s1 = inletState.s
                rho1 = inletState.rho
                p1 = inletState.p
                T1 = inletState.T
        ...
        if t<self.theta_d<t+h and self.__before_discharge__==False:
            ...
            for key in self.CVs.exists_keys:
                if self.CVs[key].discharge_becomes in self.CVs.keys:
                    ...
                    newCV.State.update({'T':oldCV.State.T,'P':oldCV.State.p})
                    oldCV.exists=False
                    newCV.exists=True
    ...
    def initial_motor_losses(self, eta_a = 0.8):
        ...
        def resid(T):
            temp.update(dict(T=T, P=outletState.p))
            return temp.s - s1

        optimize.newton(resid, temp.T)
        h2s = temp.h
