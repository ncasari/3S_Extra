...
#             self.m[:,i]=x[self.NCV:2*self.NCV]
    ...
    def guess_outlet_temp(self, inlet_state, p_outlet, eta_a=0.7):
        ...
        def objective2(T):
            outlet_state.update(dict(P=p_outlet, T=T))
            return outlet_state.h - h2

        T2 = scipy.optimize.newton(objective2, T2s)
        ...
        while True:
            ...
            if error_metric < 0.1*epsilon_cycle and np.max(np.abs(self.lumps_resid)) < epsilon_energy_balance:
                ...
                if len(self.solvers.hdisc_history) == 1:
                    ...
                    def objective(T):
                        self.Tubes.Nodes[self.key_outlet].update(dict(
                            P=self.Tubes.Nodes[self.key_outlet].p, T=T))

                        return  self.Tubes.Nodes[self.key_outlet].h- h_outlet+5

                    # Solve for isentropic temperature
                    scipy.optimize.newton(objective, self.Tubes[self.key_outlet].State2.T)
                    ...
                    def objective(T):
                        ...
                        return  self.Tubes.Nodes[self.key_outlet].h- hdnew
                    scipy.optimize.newton(objective, self.Tubes[self.key_outlet].State2.T)
