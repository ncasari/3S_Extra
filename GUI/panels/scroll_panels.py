...
class ScrollWrapAnglesFrame(wx.Frame):
    def __init__(self, geo):
        ...
        label9 = ReadOnlyLaTeXLabel('$U_1$', parent = panel, remaining_label='[rad]')
        self.U1 = wx.TextCtrl(panel)
        self.U1.SetEditable(False)
        label10 = ReadOnlyLaTeXLabel('$U_3$', parent = panel, remaining_label='[rad]')
        self.U3 = wx.TextCtrl(panel)
        self.U3.SetEditable(False)
        label11 = ReadOnlyLaTeXLabel('$U_6$', parent = panel, remaining_label='[rad]')
        self.U6 = wx.TextCtrl(panel)
        self.U6.SetEditable(False)
        ...
        phi0 = (geo.phi_i0+geo.phi_o0)/2
        self.U1.SetValue(str(geo.phi_os-phi0))
        self.U3.SetValue(str(geo.phi_is-phi0))
        self.U6.SetValue(str(geo.phi_ie-phi0))
        ...
        sizer_for_outputs.AddMany([(0,5), (0,5),        # Add some vertical space
                                   label9, self.U1,
                                   label10, self.U3,
                                   label11, self.U6])
    ...
    def __init__(self, parent, config):
        ...
        if 'disc_curves' in config:
            ...
            if 'r2' in config['disc_curves']:
                ...
                r2 = 0.0
            if 'r1' in config['disc_curves']:
                r1 = config['disc_curves']['r1']
            else:
                r1 = config['ro']-r2
        else:
            type = '2Arc'
        ...
        sizer1.Add(wx.StaticText(self,label='Radius of arc 1 [m]'))
        self.r1 = wx.TextCtrl(self, value=str(r1))

        sizer2.Add(wx.StaticText(self,label='Radius of arc 2 [m]'))
        ...
        #  parameter 
        for o in [self.type]:
            o.Bind(wx.EVT_CHOICE, self.GetGrandParent().OnRefresh)

        for o in [self.r1, self.r2]:
...
sim.set_scroll_geo(Vdisp, # Vdisp [m^3/rev]
               ...
               phi_os = phi_os, # [rad]
               phi_is = phi_is) # [rad]
sim.set_disc_geo("{disc_curves_type:s}", r1 = {disc_curves_r1:s}, r2 = {disc_curves_r2:s})
...
class GeometryPanel(pdsim_panels.PDPanel):
    ...
    def OnAnimate(self, event = None):
        ...
        SAF = ScrollAnimForm(self.Scroll.geo, size=(800,800), param_dict = self.main.get_GUI_object_value_dict(), disc_xy_coords = disc_xy_coords)
        SAF.Show()
    ...
    def OnRefresh(self, event = None):
        ...
        if self.disc_curves.type.GetStringSelection() == '2 Arcs':
            ...
            self.disc_curves.r1.Enable(False)
        elif self.disc_curves.type.GetStringSelection() == 'Arc-Line-Arc':
            disc_curves_type = 'ArcLineArc'
            self.disc_curves.r1.Enable(True)
        else:
            raise ValueError
        
        #  Get r1 and r2 as a string, convert to a floating point value if possible
        r1 = self.disc_curves.r1.GetValue()
        ...
        self.Scroll.set_disc_geo(disc_curves_type, r1 = r1, r2 = r2)
    ...
    def get_config_chunk(self):
        ...
        if disc_type == '2 Arcs':
            ...
            raise ValueError
        disc_r1 = self.disc_curves.r1.GetValue()
        disc_r2 = self.disc_curves.r2.GetValue()
        d.update(dict(disc_curves = dict(type = disc_type, r1 = disc_r1, r2 = disc_r2)))
...
class VirtualSensorsPanel(pdsim_panels.PDPanel):
    ...
    def __init__(self, parent, config, **kwargs):
        ...
        for coord in config['coords']:
            x, y = coord
            but = SuperButton(scrolled_panel,label='x = {x:g}, y = {y:g}'.format(x=x,y=y))
            but.xval = x
            but.yval = y
