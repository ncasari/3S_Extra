...
def mathtext_to_wxbitmap(s):
    # #The magic from https://matplotlib.org/stable/gallery/user_interfaces/mathtext_wx_sgskip.html#sphx-glr-gallery-user-interfaces-mathtext-wx-sgskip-py
...
class PlotPanel(wx.Panel):
    def __init__(self, parent, toolbar = False, figsize=(4,4), **kwargs):
...
class OutputTreePanel(wx.Panel):
        ...
        for key,name in [('p','Pressure (kPa)'),('T','Temperature (K)'),('rho','Density (kg per m3)'),('V','Volume (m3)')]:
            ...
            for run in self.runs:
                ...
                for c in range(0, datas[0].shape[1]-1):
                    CVkey = run.get('CVs/keys/%s' %(c,)).asstr()[()]
    ...
    def OnRightUp(self, evt):
        ...
        if col > 0: # col == 0 for the name column
            menuitem2 = wx.MenuItem(menu, -1, 'Open the HDF5 file')
            self.Bind(wx.EVT_MENU, lambda event: self.OnOpenH5File(event, col), menuitem2)
    ...
    def OnOpenH5File(self,event,col):
        try:
            os.system("start " + self.runs[col-1].filename)
        except BaseException as E:
            print("Cannot open the HDF5 file.")
            print(E)
...
class MapTable(wx.grid.Grid):
    ...
    def __init__(self, parent, values = None):
        ...
        #  Make the grid the same shape as the data
        self.CreateGrid(10, 2) # Nrows, Ncolumns

        #  Build the headers
        self.SetColLabelValue(0, 'Evap. Temp. [°C]')
        self.SetColLabelValue(1, 'Cond. Temp. [°C]')
        ...
        # Default values
        default_values = ((-30,20),
                          (5, 20),
                          (15, 30),
                          (15, 65),
                          (0, 65),
                          (-30,35))
        for i, values in enumerate(default_values):
            Te,Tc = values
            self.SetCellValue(i, 0, str(Te))
            self.SetCellValue(i, 1, str(Tc))
    ...
    def get_grid(self, step=5):
        ...
        def point_inside_polygon(point,poly):
            ...
            for i in range(1,n+1):
                ...
                if p1y==p2y:
                    ...
                    if (y >= min(p1y,p2y)) and (y <= max(p1y,p2y)) and (x==(y-p1y)*(p2x-p1x)/(p2y-p1y)+p1x):
                        inside = True
                        break
        ...
        for i in range(self.GetNumberRows()):
            try:
                Te.append(float(self.GetCellValue(i, 0)))
                Tc.append(float(self.GetCellValue(i, 1)))
            except ValueError:
                pass
        polygon = list(zip(Te,Tc))
        Te_min = min(Te)
        Te_max = max(Te)
        Tc_min = min(Tc)
        Tc_max = max(Tc)

        # Create array from Te_min/Tc_min to Te_max/Tc_max given a step
        vTe = np.arange(Te_min, Te_max, step)
        vTc = np.arange(Tc_min, Tc_max, step)
        if vTe[-1]!=Te_max:
            vTe = np.append(vTe, Te_max)
        if vTc[-1]!=Tc_max:
            vTc = np.append(vTc, Tc_max)

        # Create the grid of points using vTe and vTc
        grid = np.meshgrid(vTe,vTc)
        vgrid = list(zip(*(x.flat for x in grid)))

        # Add points on the map boundaries
        n = len(polygon)
        p1x,p1y = polygon[0]
        for i in range(1,n++1):
            p2x,p2y = polygon[i % n]
        ...
        # Keep only point inside the map
        result = []
        for point in vgrid:
...
class MapSelection(wx.Dialog):
    def __init__(self, parent, values = None):
        wx.Dialog.__init__(self, parent, title = 'Defintion of the Operating Map')

        self.OKButton = wx.Button(self,label='OK')
        self.OKButton.Bind(wx.EVT_BUTTON, lambda event: self.EndModal(wx.ID_OK))
        self.MAP_coords = MapTable(self)

        sizer = wx.BoxSizer(wx.VERTICAL)
        sizer.Add(self.OKButton, proportion = 0, flag=wx.EXPAND)
        sizer.Add(self.MAP_coords, proportion = 1, flag=wx.EXPAND)
        self.SetSizer(sizer)
        sizer.Layout()
        #
        w,h = self.GetEffectiveMinSize()
        self.SetSizeWH(w+20,w)

        self.MAP_coords.ForceRefresh()
        self.Refresh()
...
class ParametricPanel(PDPanel):
    ...
    def __init__(self, parent, configdict, **kwargs):
        ...
        self.RunOnMap = HackedButton(self._mb, label = 'Run on Map')
        self._mb.AddControl(self.RunOnMap)
        self.RunOnMap.Bind(wx.EVT_BUTTON, self.OnRunOnMap)
        # self.FullMap.Disable()
...
#            self.OnBuildTable()
    ...
    def OnRunOnMap(self, event = None):
        dlg = MapSelection(None)
        if dlg.ShowModal() == wx.ID_OK:
            # grid = dlg.MAP_coords.get_grid()
            self.map_coords = dlg.MAP_coords.get_grid()
            self.BuildRunOnMap()
        dlg.Destroy()

    def BuildRunOnMap(self):
        self.OnAddTerm()
        self.OnAddTerm()
        self.OnAddTerm()

        label = self.ParamSizer.GetChildren()[0].Window.Terms.Items
        self.ParamSizer.GetChildren()[0].Window.Terms.SetSelection(label.index(u'Suction saturation temperature [K]'))
        self.ParamSizer.GetChildren()[1].Window.Terms.SetSelection(label.index(u'Discharge saturation temperature [K]'))
        self.ParamSizer.GetChildren()[2].Window.Terms.SetSelection(label.index(u'Suction superheat [K]'))

        self.Structured.SetValue(False)
        self.OnChangeStructured()
        nrows = len(self.map_coords)
        ...
        for i in range(nrows):
            ...
            self.ParaList.SetCellValue(i,0,str(self.map_coords[i][0]+273.15))
            self.ParaList.SetCellValue(i,1,str(self.map_coords[i][1]+273.15))
            self.ParaList.SetCellValue(i,2,str(10))

        self.RowCountSpinner.SetValue(nrows)
        self.RowCountSpinnerText.SetValue(str(nrows))
...
#            #Build and add a sizer for the para values
    ...
    def build_all_scripts(self):
        ...
        for Irow in range(self.ParaList.GetItemCount()):
            ...
            if self.ParaList.IsItemChecked(Irow):
...
class StateChooser(wx.Dialog):
    ...
    def __init__(self,Fluid,T,rho,parent=None,id=-1,Fluid_fixed = False):
        ...
        self.Fluids.AppendItems(sorted(CoolProp.CoolProp.get_global_param_string('predefined_mixtures').split(',')))
        ...
        self.rholabel, self.rho = LabeledItem(self,label="Density [kg/m\xb3]",value='1',enabled=False)
...
class StatePanel(wx.Panel):
    ...
    def UseChooser(self,event=None):
        ...
        SCfrm=StateChooser(Fluid=Fluid,T=T,p=p,Fluid_fixed = self._Fluid_fixed)
...
class StateInputsPanel(PDPanel):
    ...
    def __init__(self, parent, config, **kwargs):
        ...
        if (('T' in config['inletState']) and ('rho' in config['inletState'])):
            inletState = State(config['inletState']['Fluid'], dict(T = config['inletState']['T'], D = config['inletState']['rho']))
        elif (('Tsat' in config['inletState']) and ('SH' in config['inletState'])):
            # Sat temperature and superheat are given
            inletState = CP.State(config['inletState']['Fluid'], dict(T=config['inletState']['Tsat'], Q=1))
            inletState = CP.State(config['inletState']['Fluid'], dict(T=config['inletState']['Tsat'] + config['inletState']['SH'], P=inletState.p))
            inletState = State(config['inletState']['Fluid'], dict(T = config['inletState']['Tsat'] + config['inletState']['SH'],
                                                                   D = inletState.rho))
        ...
        elif 'Tsat' in config['discharge']:
            Tsat = config['discharge']['Tsat']
            try:
                state = CP.State(inletState.Fluid, dict(T=Tsat,Q=1))
                pressure = state.p
                pratio = pressure / inletState.p
            except ValueError:
                raise ValueError('Discharge saturated temperature are not good...')
            self.radio_discTsat.SetValue(True)
        else:
    ...
    def get_config_chunk(self):
        ...
        configdict['inletState'] = dict(Fluid = inletState.Fluid.decode('utf-8'),
                                        T = inletState.T,
                                        rho = inletState.rho)
