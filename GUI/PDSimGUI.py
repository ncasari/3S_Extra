...
from DanfossPDSim.plot.plots import PlotNotebook
import PDSim
...
class RunToolBook(wx.Panel):
    def __init__(self,parent):
        ...
        splitter.SplitHorizontally(top_pane, bottom_pane, -300)
...
class MainToolBook(wx.Toolbook):
    def __init__(self, parent, configdict):
        ...
        for imgfile in ['Inputs.png', 'Solver.png', 'Solver.png', 'Outputs.png']:
            ico_path = os.path.join('ico',imgfile)
            if ico_path.endswith('.png'):
                bmp = wx.Image(ico_path, wx.BITMAP_TYPE_PNG).ConvertToBitmap()
                indices.append(il.Add(bmp))
            else:
                bmp = wx.svg.SVGimage.CreateFromFile(ico_path).ConvertToScaledBitmap(wx.Size(32,32), self)
                indices.append(il.Add(bmp))
        self.AssignImageList(il)
...
class MainFrame(wx.Frame):
    def __init__(self, configfile = None, position = None, size = None):
        wx.Frame.__init__(self, None, size = (700, 700),style=wx.DEFAULT_FRAME_STYLE |
        ...
        if configfile is None: #No file name or object passed in
            ...
            # Then use the internal default scroll
                ...
                self.config = default_configs.get_defaults('danfoss')
    ...
    def script_header(self):
        import CoolProp, PDSim, DanfossPDSim
        time_generation = time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime())
        danfosspdsim_version = DanfossPDSim.__version__+' git branch: '+str(DanfossPDSim.__git_branch__)+' git revision: '+str(DanfossPDSim.__git_revision__)
        ...
        return textwrap.dedent(
            ...
            # Versions used:
            # DanfossPDSim: {danfosspdsim_version:s}
    ...
    def build(self):
        ...
        # Set the title
        self.SetTitle("3S GUI - Replay Branch - (" + self.config.get('title', 'Default') + ")")
    ...
    def load_families(self, FamiliesMenu):
        ...
        for mod in load_all_modules_from_dir('families'):
            ...
            if mod.family_menu_name == self.machinefamily:
                menuItem.Check(True)
    ...
    def OnConfigOpen(self,event):
        ...
        if wx.ID_OK==FD.ShowModal():
            ...
            try:
                configdict = yaml.load(codecs.open(file_path,'r',encoding='utf-8'),Loader=yaml.FullLoader)
            except UnicodeDecodeError:
                configdict = yaml.load(codecs.open(file_path,'r',encoding='latin-1'),Loader=yaml.FullLoader)

            configdict['title'] = file_path
...
if __name__ == '__main__':
    ...
    if '--onecycle' in sys.argv:
        frame.MTB.SolverTB.SolverPanel.OneCycle.SetValue(True)

    if '--devrun' in sys.argv:
        frame.MTB.SetSelection(2)
        script_name = frame.build_simulation_script(description = '')
        frame.run_batch([script_name])
